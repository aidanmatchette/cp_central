...
function RandomPersonGenerator() {
  ...
  const handleRandomPerson = () => {
    ...
    backend.get('api/instructor/checkin/').then(response => {
      let randSelection = response.data[Math.floor(Math.random()*response.data.length)]
  ...
  return (
    <ThemeProvider theme={theme}>
      <Button onClick={handleRandomPerson} variant="contained" sx={{width: '80%', mt: 3}} >Random Person</Button>
      <Modal
        keepMounted
        open={isModalOpen}
        ...
        <Box style={style} sx={{backgroundColor: 'white'}}>
          <Typography id="keep-mounted-modal-title" variant="h5" component="h2" sx={{textAlign: 'center', fontWeight: 700}}>
            Random Person 
          </Typography>
          <Typography id="keep-mounted-modal-description" sx={{ mt: 2 , mb: 2, textAlign: 'center'}}>
